...
veryOldInventors = inventors.filter((inventors) => {
    return inventors.year < 1600 && inventors.year > 1499;
});
...
// people.forEach((x) => {
//     firstLast.push(x.split(','));
// });

firstLast = people.map((element) => element.split(','));

firstLast = firstLast.map((element) => `${element[1]} ${element[0]}`);
...
// devs.forEach((x) => {
//     if(2024 - x.year >= 18){
//         console.log(2024 - x.year);
//     } else {
//         console.log('This a Minor');
//     }
// })

isAdultPresent = devs.some((element) => 2024 - element.year >= 18);
...
isEveryone19OrOlder = devs.every((element) => 2024 - element.year >= 19);
...
totalYearsLived = inventors.reduce((sum, element) => {
    return element.passed - element.year + sum;
}, 0);
...
// travelMethodCounts = travelMethods.reduce(function (sum, toy) {
// }, {});

travelMethodCounts = travelMethods.reduce((sum, toy)=> {
    if (sum[toy]) {
        sum[toy]++;
    } else {
        sum[toy] = 1;
